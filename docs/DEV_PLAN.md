...
1. **标签系统** (P0)
   ...
   - **文件**: `src/types/`, `src/components/TagManager.tsx`
...
1. **后端服务** (P0)
   - [ ] 选择后端技术栈（Node.js + Express / Nest.js）
   - [ ] 设计 RESTful API
   - [ ] 实现用户认证（JWT）
   - [ ] 实现笔记 CRUD API
...
2. **前端集成** (P0)
   - [ ] 集成 API 客户端（axios / fetch）
   - [ ] 实现登录/注册功能
   - [ ] 实现数据同步逻辑
   - [ ] 处理离线模式
   - [ ] 冲突解决策略
   - **文件**: `src/api/`, `src/hooks/useSync.ts`
